# UMBRAMED Grant Applications Status — 17 March 2026

## Active Grant Applications

### 1. EIC Accelerator 2026 Step 1 ✅ SUBMITTED
- **Call**: HORIZON-EIC-2026-ACCELERATOR-01
- **Final ID**: 101335011
- **Draft ID**: SEP-211312472
- **Status**: Submitted and confirmed
- **Funding**: Up to €2.5M equity + €2.5M grant
- **Success rate**: 15-25% (Step 1 short application)
- **Applicant**: Agents AI Ltd (PIC: 869706366)
- **Next**: Wait for Step 1 evaluation results (typically 6-8 weeks)

### 2. HORIZON-HLTH-2026-01-CARE-03 (Low-Value Care) 🚫 BLOCKED
- **Call**: HORIZON-HLTH-2026-01 (Cluster 1 - Health, Single stage 2026)
- **Topic**: Identifying and eliminating low-value care
- **Type of Action**: HORIZON-RIA (Research and Innovation Action)
- **Draft ID**: SEP-211312838
- **Acronym**: UMBRAMED-LVC
- **Proposal Title**: AI-Assisted Elimination of Low-Value Administrative Care in European Primary Healthcare
- **Deadline**: 16 April 2026, 17:00 Brussels Local Time
- **Funding**: Up to €10M (topic budget)
- **Duration**: 48 months
- **Portal Account**: haoyunzhu888@gmail.com

#### Consortium Partners (current):
| # | Organisation | Country | PIC | Role | Main Contact |
|---|-------------|---------|-----|------|-------------|
| 1 | Agents AI Ltd | UK | 869706366 | Coordinator | Sami Halawa Ribas |
| 2 | EMPLEADOS AI | Spain (Madrid) | 871948036 | Partner | Valerio Trigos |

#### ⚠️ CRITICAL: Need 3rd Partner
- RIA requires minimum 3 entities from 3 different EU/Associated countries
- UK counts as Associated Country (post-Brexit)
- Spain is EU Member State (satisfies the EU requirement)
- **Need 1 more partner from a 3rd country** (e.g., Germany, Italy, Netherlands, Portugal)
- Options: Post Partner Search on EU portal, contact academic health informatics groups

#### Part A Administrative Form — Completed Fields:
- ✅ Acronym: UMBRAMED-LVC
- ✅ Proposal title
- ✅ Duration: 48 months
- ✅ Abstract/Short Summary
- ✅ Free keywords
- ✅ Fixed keyword: Health services, health care research
- ⬜ Budget tables (per partner)
- ⬜ Ethical issues self-assessment

#### Part B Technical Description — TO DO:
- ⬜ Part B PDF (technical proposal document)
- ⬜ Clinical studies annex
- ⬜ Part B Ethics section

---

### 3. AI Champions: Frontier AI Phase 1 (Innovate UK) 📝 APPLICATION READY
- **Competition ID**: 2419
- **Portal**: apply-for-innovation-funding.service.gov.uk
- **Status**: Full application drafted, appendices prepared, waiting only for portal login and final submit
- **Opens**: 17 March 2026 | **Closes**: 29 April 2026, 11:00am
- **Funding**: Up to £250,000 (70% grant = £175,000)
- **Type**: Feasibility study, single UK SME
- **Theme**: Theme 1 — AI-Enabled Health and Life Sciences → predictive healthcare
- **Project Title**: UMBRAMED: Hybrid Safety-Constrained Clinical LLM for Autonomous Prescription Renewal
- **Duration**: 6 months (1 Aug 2026 – 31 Jan 2027)
- **Applicant**: Agents AI Ltd (London, UK)
- **Application Document**: `AI_CHAMPIONS_FRONTIER_AI_PHASE1_APPLICATION.html`
- **Appendices Needed**: Architecture diagram PDF, Team CVs PDF, Gantt chart PDF, Risk register PDF
- **⚠️ ACTION**: Sign in to IFS portal and submit when competition opens. User must enter password themselves.

---

### 4. Eurostars Call 10 (Eureka Network) 🟡 LIVE BUT FAST-TURNAROUND
- **Programme**: Eurostars-3 Call 10
- **Status**: Open now
- **Window**: 16 January 2026 to 19 March 2026, 14:00 Brussels time
- **Fit**: Potentially strong if Agents AI Ltd and the Spanish SME are both eligible and independent
- **Why it matters**: International SME R&D project, health AI is eligible, and both Spain and the United Kingdom have national funding allocated for this call

#### Key eligibility points from official sources:
- Consortium must be led by an innovative SME from a Eurostars country
- Consortium must include at least two independent entities from at least two Eurostars countries
- At least one organisation must come from an EU Member State or Horizon Europe Associated Country
- SME share of the budget must be at least 50% of total project cost
- No single participant or country can carry more than 70% of total budget
- Project duration must be 36 months or less
- Civil applications only

#### Country-specific funding notes:
- **United Kingdom**: Innovate UK funds SME R&D costs up to 60%; UK funded participants must be active at Companies House for at least 12 months before the deadline
- **Spain**: CDTI funds Spanish SMEs up to 60% for small companies or 50% for medium-sized companies, possibly up to 70% if national budget allows; the Spanish SME must have deposited at least the latest annual accounts by the deadline

#### Practical blocker:
- This is only a realistic submission if the Spanish entity is an eligible SME with filed accounts and the two-company consortium can assemble the required Eurostars application immediately.
- Local machine review on 17 March 2026 surfaced only strategy notes and backlog references, not the proof pack Eurostars needs.
- `Corporate Structure & Branding (Company Registration)` in the Notion export is only a lesson stub.
- `empleadosai` in the Notion kanban is explicitly marked `Backlog` and `Speculative`.
- No local filing evidence was found for latest Spanish annual accounts, applicant incorporation proof for the exact pair, or a draft Eurostars submission package.
- **Decision**: Do not treat Eurostars as ready unless company-accounting evidence is found before the deadline.

---

### 5. EIT Health Innovation Validation Call 2026 🟡 OPEN BUT CONSORTIUM-BUILD
- **Status**: Open
- **Opens**: 25 February 2026
- **Closes**: 6 May 2026
- **Funding**: Up to EUR 850,000 per project, up to 50% of total project budget
- **Fit**: Strong thematic fit for UMBRAMED if the product is mature enough for clinical validation and regulatory preparation

#### Official eligibility highlights:
- Consortium of at least two organisations from different countries
- Consortium must represent at least two sides of the Knowledge Triangle: industry, research, education
- Commercialising company must lead
- Solution must be a digital or AI-enabled medical device at Innovation Maturity Level 6, aiming for IML 7 by project end

#### Practical blocker:
- Current repo does not show a ready second-country research/education partner or a submission-ready maturity evidence pack for IML 6, so this is open but not immediate.
- Local machine review on 17 March 2026 did not surface a committed research or education partner from another country, an IML 6 evidence dossier, or a draft package comparable to the AI Champions materials.
- **Decision**: Keep EIT Health in the next-wave pipeline, not the immediate submission queue.

---

## Pending / Future Opportunities

### 3. EIC Pathfinder Open 2026
- **Deadline**: 12 May 2026
- **Funding**: Up to €3M
- **Type**: Consortium or single entity
- **Fit**: Lower — more suited for breakthrough science, but could frame UMBRAMED as novel AI-clinical protocol integration research

### 4. Eurostars Call 10
- **Deadline**: 19 March 2026 (very close!)
- **Requirements**: 2 entities from 2 Eureka countries
- **Funding**: Variable by country
- **Note**: Tight timeline, may not be feasible for this round

### 5. CDTI NEOTEC 2026 (Spain)
- **Funding**: Up to €325K
- **Requirements**: Spanish company <3 years old, with a doctor on team
- **Note**: EMPLEADOS AI could apply if it meets the age requirement

---

## Key Contacts & Credentials

| Person | Role | Email |
|--------|------|-------|
| Sami Halawa Ribas | Project Coordinator, CEO Agents AI Ltd | samihalawaster@gmail.com |
| Dr. Valerio Trigos | Clinical Lead, 6,500 patients in Diraya | valerio.trigos88@gmail.com |

### EU Portal
- **Account**: haoyunzhu888@gmail.com
- **Organisations**: Agents AI Ltd (PIC: 869706366), EMPLEADOS AI (PIC: 871948036)

---

## Immediate Next Steps (Priority Order)

1. **Submit AI Champions Frontier AI Phase 1**
   - Use `AI_CHAMPIONS_FRONTIER_AI_PHASE1_APPLICATION.html` as the canonical application draft
   - Upload the 4 prepared appendix PDFs from `output/pdf/`
   - Complete portal submission after login

2. **Attempt Eurostars Call 10 only if both SME entities are eligible today**
   - Confirm the Spanish company is an SME with filed latest annual accounts
   - Confirm independence of the UK and Spanish entities
   - Build and submit only if those legal/financial checks pass before 19 March 2026

3. **Find 3rd consortium partner** for CARE-03 (from a 3rd country)
   - Post Partner Search announcement on EU F&T Portal
   - Reach out to health informatics research groups in Germany, Italy, Netherlands, or Portugal

4. **Assess EIT Health Innovation Validation Call 2026 as the next serious option**
   - Requires a different-country research or education partner and IML 6 evidence
   - Strong fit, but not ready from current repo assets alone

5. **Turn CARE-03 from blocked to eligible**
   - Keep `CARE03_PARTB_DRAFT.md` as the working narrative
   - Finalise budget and ethics only after the third partner is confirmed
   - Do not treat CARE-03 as ready to submit before consortium eligibility exists

6. **Monitor EIC Step 1**
   - No new submission action required unless invited to the next stage

---

## Evidence From Local Machine Review (17 March 2026)

Strongest off-repo sources reviewed today:

- `.../NOTION-FULL-EXPORT-2026-3-16/Private & Shared/WhatsApp Chat - VALERIO ALUMNO AI Umbramed (3) Imp/00000302-Hoja_de_Ruta_de_Financiacion_Definitiva_para_Valerio_.pdf`
- `.../NOTION-FULL-EXPORT-2026-3-16/Private & Shared/WhatsApp Chat - VALERIO ALUMNO AI Umbramed (3) Imp/00000303-Oportunidades_de_Financiacion_para_el_Proyecto_de_Salud_Digital_de_Valerio_(1).pdf`
- `.../NOTION-FULL-EXPORT-2026-3-16/Private & Shared/WhatsApp Chat - VALERIO ALUMNO AI Umbramed (3) Imp/00000304-Valerio_Salud_Digital___Financiacion_Estrategica.pdf`
- `.../NOTION-FULL-EXPORT-2026-3-16/Umbramed/.../Valerio's AI Course/Corporate Structure & Branding (Company Registration)...`
- `.../NOTION-FULL-EXPORT-2026-3-16/Private & Shared/Cloudflare Projects Kanban/empleadosai ... .md`

What those sources changed:

- They confirm prior strategic thinking around Spanish grants, ENISA, RedIA, AES, and future EU expansion.
- They do **not** provide hidden submission-ready evidence for Eurostars, EIT Health, or CARE-03.
- They reinforce that `AI Champions` remains the only remaining application lane already assembled to near-submit level.

---

*Last updated: 17 March 2026*
